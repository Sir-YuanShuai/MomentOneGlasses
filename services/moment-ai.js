...
import {
  ...
  MOMENT_UNDERSTANDING_PROMPT
} from '../prompts/moment-understanding-v1.js';

const ALLOWED_CATEGORIES = Object.keys(CATEGORY_META);

export const MOMENT_SYSTEM_PROMPT = MOMENT_UNDERSTANDING_PROMPT.system;
export const MOMENT_PROMPT_VERSION = `${MOMENT_UNDERSTANDING_PROMPT.id}@${MOMENT_UNDERSTANDING_PROMPT.version}`;
...
export async function answerMemoryQuestion(question, moments) {
  ...
  let session;
  try {
    session = await createSession(MEMORY_ANSWER_PROMPT.system);
