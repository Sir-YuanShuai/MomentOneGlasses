...
//#endregion
//#region scripts/qr-fallback-source.js
var import_jsQR = /* @__PURE__ */ __toESM(require_jsQR(), 1);
function decodeQrPixels(data, width, height) {
	if (!data || !Number.isFinite(width) || !Number.isFinite(height)) return null;
	const pixels = data instanceof Uint8ClampedArray ? data : data instanceof ArrayBuffer ? new Uint8ClampedArray(data) : ArrayBuffer.isView(data) ? new Uint8ClampedArray(data.buffer, data.byteOffset, data.byteLength) : null;
	if (!pixels || pixels.byteLength < width * height * 4) return null;
	const result = (0, import_jsQR.default)(pixels, width, height, { inversionAttempts: "attemptBoth" });
	if (!result || typeof result.data !== "string" || !result.data) return null;
	return result.data;
}

//#endregion
export { decodeQrPixels };