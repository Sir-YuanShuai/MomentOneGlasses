...
- `app.js`：应用生命周期及全局配置。
- `pages/welcome/welcome.ink`：应用入口欢迎页，根据绑定状态分流。
- `pages/scan/scan.ink`：扫码绑定页，BarcodeDetector + 相机换取 token。
- `pages/index/index.ink`：Moment 记录入口，统一语音入口识别记录/查询/回顾/设置意图。
...
- `services/binding.js`：设备绑定与 token 校验（对接 MomentOneServer）。
